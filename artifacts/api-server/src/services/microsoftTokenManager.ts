...
export async function getMicrosoftAccessToken(
  ...
  const params = new URLSearchParams({
    ...
    scope: "Mail.Read Calendars.Read Contacts.Read User.Read offline_access",
  });
  ...
  const { access_token, expires_in, error: tokenError, error_description } = response.data;

  if (!access_token) {
    const errMsg = error_description || tokenError || "Failed to obtain Microsoft access token";
    console.error("Microsoft token refresh failed:", JSON.stringify(response.data));
    if (tokenError === "invalid_grant") {
      throw new Error("Microsoft authorization expired. Please disconnect and reconnect Microsoft in Connected Services.");
    }
